#!/bin/bash
# Builds PressureClutch.app from the SwiftPM package.
#
...
APP="build/PressureClutch.app"

echo "▸ Compiling…"
swift build --configuration release --disable-sandbox
BIN="$(swift build --configuration release --disable-sandbox --show-bin-path)/PressureClutch"

echo "▸ Bundling ${APP}…"
rm -rf "$APP"
mkdir -p "${APP}/Contents/MacOS"
cp "$BIN" "${APP}/Contents/MacOS/PressureClutch"
...
if [[ $RUN -eq 1 ]]; then
    pkill -x PressureClutch 2>/dev/null || true
    open "$APP"
fi
